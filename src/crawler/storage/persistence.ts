  ...
  async upsertProduct(item: NormalizedItem, brandId: string): Promise<string> {
    const result = await this.sql`
      INSERT INTO products (
        canonical_name, display_name, brand_id, pit_type, category, sub_category,
        ...
        ${item.canonicalName}, ${item.displayName}, ${brandId}, ${item.pitType}, ${item.category}, ${item.subCategory},
      ...
      ON CONFLICT (source_platform, external_id) WHERE external_id != '' AND deleted_at IS NULL
      DO UPDATE SET
