...
/** 公开前缀：与 bucket policy 放行的匿名读前缀对齐（feed/），圈子动态图走 img 域名直出 */
const PUBLIC_OBJECT_PREFIX = 'feed/';
/** 私有前缀：purchase_import / ai_import / wardrobe 等含用户订单隐私，仅 API 代理读 */
const PRIVATE_OBJECT_PREFIX = 'private/';

export function buildObjectKey(userId: string, purpose: string, objectId: string): string {
  return purpose === 'outfit'
    ? `${PUBLIC_OBJECT_PREFIX}${userId}/${objectId}`
    : `${PRIVATE_OBJECT_PREFIX}${userId}/${purpose}/${objectId}`;
}
...
export async function registerUploadRoutes(
  ...
  // OSS 模式下公开图 302 到 img 域名直出（省服务器带宽）；其余走 API 代理（私有图不透传）。
  app.get('/api/v1/media/:mediaId', async (request, reply) => {
    ...
    const publicUrl = storage.publicUrl(media.objectKey);
    if (publicUrl) return reply.redirect(publicUrl, 302);
  ...
  app.post('/api/v1/uploads:prepare', async (request) => {
    ...
    const objectKey = buildObjectKey(userId, body.purpose, newId('obj'));
